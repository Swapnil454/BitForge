...
export default function BuyerFooter() {
  ...
  return (
    <footer className="bg-white dark:bg-[#0A1628] border-t border-gray-100 dark:border-slate-800/60 pt-12 pb-6 transition-colors duration-200">
      <div className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-8 lg:gap-10 mb-12">
          ...
          <div className="col-span-2 md:col-span-2 flex flex-col gap-5">
            {/* Logo + wordmark */}
            <Link href="/marketplace" className="flex items-center gap-3 w-fit group">
              <div className="w-54 h-54 -ml-13 -mt-15 relative flex-shrink-0">
                <Image
              ...
              <span className="text-3xl -ml-10 -mt-15 font-extrabold tracking-tight text-gray-900 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">
                Bitforge
              </span>
            </Link>

            {/* Tagline */}
            <p className="text-sm -mt-15 text-gray-500 dark:text-slate-400 leading-relaxed max-w-[260px]">
            ...
            <div className="flex flex-wrap gap-2">
              <span className="text-[11px] font-semibold px-3 py-1 rounded-full border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400">
                Secure Checkout
              </span>
              <span className="text-[11px] font-semibold px-3 py-1 rounded-full border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400">
                99.9% Uptime
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-3">
              {footerLinks.company.map((l) => (
                <li key={l.href}>
                  <Link href={l.href} className="text-sm text-gray-600 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors">
                    {l.label}
                  </Link>
                </li>
              ))}
              <li>
                <Link href={helpCenterHref} className="text-sm text-gray-600 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors">
                  Help Center
                </Link>
              </li>
            </ul>
          ...
          <div className="flex flex-col gap-4">
            ...
            <ul className="flex flex-col gap-3">
              ...
              {isAuthenticated && (
                <li>
                  <Link href="/wishlist" className="text-sm text-gray-600 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors">
                    My Wishlist
                  </Link>
                </li>
              )}
        ...
        {/* ── Bottom bar ── */}
        <div className="border-t border-gray-100 dark:border-slate-800/60 pt-5 flex flex-col sm:flex-row justify-between items-center gap-2">
          <p className="text-xs text-gray-400 dark:text-slate-500">
            © {new Date().getFullYear()} Bitforge Technologies. All rights reserved.
          </p>
          <div className="flex items-center gap-5">
            <Link href="/legal/privacy-policy"        className="text-xs text-gray-400 dark:text-slate-500 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors">Privacy</Link>
            <Link href="/legal/terms-and-conditions"  className="text-xs text-gray-400 dark:text-slate-500 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors">Terms</Link>
            <Link href="/status"                      className="text-xs text-gray-400 dark:text-slate-500 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors">Status</Link>
          </div>
        </div>
