    ...
    <div className="py-10 px-4 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <section className="mb-10">
          ...
          <div className="bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-6">
            <h3 className="text-slate-900 dark:text-white font-semibold mb-3">Environment Variable Example</h3>
            <CodeBlock
              code={`# .env (add to .gitignore)
Bitforge_API_KEY=csfy_live_abc123...
Bitforge_SECRET_KEY=sk_live_xyz789...
        ...
        <section className="mb-10">
          ...
          <div className="space-y-4">
            <div className="bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-6">
              ...
              <CodeBlock
                code={`// ✓ Good - HTTPS
fetch('https://api.Bitforge.com/products', {
  headers: {
...
//  Bad - HTTP (will fail)
fetch('http://api.Bitforge.com/products', ...)`}
                language="javascript"
              />
            </div>
        ...
        <section className="mb-10">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-5">
              ...
              <p className="text-slate-500 dark:text-white/60 text-sm">
                Only collect customer data you need. Don't store sensitive information unnecessarily. Use Bitforge's built-in customer management.
              </p>
            </div>
            <div className="bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-5">
              <h3 className="text-slate-900 dark:text-white font-semibold mb-2 flex items-center gap-2">
...
if (!verifyWebhook(req.body, req.headers['x-webhook-signature'], secret)) {
  return res.status(401).json({ error: 'Invalid signature' });
        ...
        {/* Security Checklist */}
        <section className="mb-10">
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">Security Checklist</h2>
          <div className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/30 rounded-xl p-6">
            <div className="space-y-3">
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" className="mt-1" />
                <span className="text-slate-700 dark:text-white/80">API keys stored in environment variables, not in code</span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" className="mt-1" />
                <span className="text-slate-700 dark:text-white/80">Using HTTPS for all API requests</span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" className="mt-1" />
                <span className="text-slate-700 dark:text-white/80">Webhook signatures verified on all incoming webhooks</span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" className="mt-1" />
                <span className="text-slate-700 dark:text-white/80">Separate API keys for development and production</span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" className="mt-1" />
                <span className="text-slate-700 dark:text-white/80">OAuth tokens refresh automatically before expiration</span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" className="mt-1" />
                <span className="text-slate-700 dark:text-white/80">Customer data handled according to GDPR requirements</span>
              </label>
            </div>
          </div>
        </section>
  ...
  return (
    <div className="relative">
      <div className="absolute top-3 right-3 z-10">
        <button
          onClick={handleCopy}
          className="px-3 py-1 bg-slate-200 dark:bg-white/10 hover:bg-white/20 border border-slate-300 dark:border-white/20 rounded text-xs text-slate-700 dark:text-white/80 transition-colors"
        >
          {copied ? "Copied!" : "Copy"}
        </button>
      </div>
