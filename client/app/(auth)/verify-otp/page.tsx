...
function VerifyOtpPageContent() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#05050a] text-slate-900 dark:text-white flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className={`relative z-10 w-full max-w-md rounded-3xl p-8 bg-slate-100 dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 shadow-lg text-center ${shake ? "animate-[shake_0.4s]" : ""}`}>
        ...
        <div className="flex justify-center gap-2 mt-8 mb-6">
          {otp.map((digit, index) => (
            <input
              ...
              maxLength={1}
              inputMode="numeric"
              className="w-11 h-12 text-center text-lg font-bold rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 outline-none focus:border-cyan-400 focus:shadow-lg transition"
            />
          ))}
        </div>
        ...
        <div className="flex justify-between text-sm mt-6 text-slate-500 dark:text-white/60">
          <button
            onClick={handleResendOtp}
            disabled={resendDisabled}
            className={`font-semibold ${resendDisabled ? "text-slate-300 dark:text-white/30 cursor-not-allowed" : "text-cyan-400 hover:underline"}`}
          >
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-white dark:bg-black/70">
          <div className="px-10 py-8 rounded-3xl bg-slate-200 dark:bg-white/10 backdrop-blur-xl border border-slate-200 dark:border-white/10 text-center shadow-lg">
            <div className="text-4xl mb-3"></div>
            <p className="text-xl font-black text-cyan-400">Email verified!</p>
            <p className="text-sm text-slate-500 dark:text-white/60 mt-1">Redirecting to dashboard…</p>
          </div>
        </div>
      )}
