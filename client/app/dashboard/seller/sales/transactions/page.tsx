...
function SellerTransactionsPageContent() {
  ...
  const [month, setMonth] = useState(period === "month" ? currentMonthStr : "all");
  ...
  }, [status, debouncedSearch, sortBy, period, month]);
              ...
              {transactions.map((transaction, index) => {
                ...
                return (
                  <motion.article
                    ...
                    className="bg-white dark:bg-[#12141c] border border-slate-200 dark:border-white/5 rounded-2xl p-4 sm:p-5 flex flex-col group hover:border-slate-300 dark:hover:border-white/10 transition-all shadow-sm hover:shadow-xl dark:shadow-none dark:hover:shadow-black/50 cursor-pointer"
                  >
                    <div className="flex gap-4 sm:gap-5 items-center">
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5 relative">
                        <div>
                          ...
                          <div className="text-[11px] text-slate-400 dark:text-slate-500 font-mono truncate mb-3 sm:mb-0 flex items-center gap-1.5">
                            Order #{transaction.orderId.slice(-8).toUpperCase()}
                            <button
                              type="button"
                          ...
                          <div className="flex items-center justify-between gap-2 min-w-0">
                            ...
                            <div className="text-right">
                              <p className="text-[11px] sm:text-xs text-slate-500 dark:text-zinc-400">
                                Bought by <span className="font-bold text-slate-800 dark:text-zinc-200">{transaction.buyerName}</span>
                              </p>
                              <p className="text-[10px] sm:text-xs text-emerald-600 dark:text-emerald-400 font-bold mt-0.5">
                                Earned ₹{transaction.netAmount.toLocaleString()}
                              </p>
                            </div>
                          </div>
                        </div>
