        ...
        <>
          ...
          >
              ...
              <section>
                ...
                <div className="space-y-4">
                  <div className="flex gap-4">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-900 dark:text-white">
                        {transaction.type === "admin_to_seller" ? "BitForge Settlement Account" : transaction.type === "seller_to_admin" ? (transaction.sellerName || "Unknown Seller") : (transaction.buyerName || "Unknown Buyer")}
                      </p>
                      {transaction.type === "buyer_to_admin" && transaction.buyerEmail && (
                        <p className="text-xs text-slate-500 dark:text-white/60">{transaction.buyerEmail}</p>
                      )}
                      {transaction.type === "seller_to_admin" && transaction.sellerEmail && (
                  ...
                  <div className="flex gap-4">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-900 dark:text-white">
                        {transaction.type === "buyer_to_admin" || transaction.type === "seller_to_admin" ? "BitForge Settlement Account" : (transaction.sellerName || "Unknown Seller")}
                      </p>
                      {transaction.type === "admin_to_seller" && transaction.sellerEmail && (
                        <p className="text-xs text-slate-500 dark:text-white/60">{transaction.sellerEmail}</p>
                      )}
                    </div>
                  </div>
            ...
            <div className="p-6 border-t border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.02] grid grid-cols-1 gap-3">
              {transaction.razorpayPaymentId && (
                <a
                  href={`https://dashboard.razorpay.com/app/payments/${transaction.razorpayPaymentId}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-medium text-sm transition"
                >
                  <ExternalLink className="h-4 w-4" />
                  View in Razorpay
                </a>
              )}
              {/* Note: Download receipt descoped for V1 
              <button className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 text-slate-700 dark:text-white font-medium text-sm transition">
