          ...
          <>
            ...
            <div className="sm:hidden relative z-50">
              ...
              <div className={`absolute right-0 top-full mt-2 w-56 bg-white dark:bg-[#0a0a14] border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl transition-all z-50 p-2 flex flex-col gap-1 ${mobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
                <button
                  onClick={() => { setMobileMenuOpen(false); router.push("/dashboard/seller/transactions/analytics"); }}
          ...
          <div className="overflow-x-auto relative">
            <table className="w-full text-left border-collapse min-w-max sm:min-w-[1000px]">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                {loading ? (
                  <tr>
                    <td colSpan={7} className="py-20 text-center text-slate-400">Loading transactions...</td>
                  </tr>
                ) : transactions.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="py-20 text-center text-slate-400">No transactions found for the current filters.</td>
                  </tr>
                ) : (
                  transactions.map(t => (
                    <tr 
                      ...
                      <td className={`px-1.5 sm:px-4 ${rowHeightClass}`}>
                        <div className="flex flex-col">
                          <span className="text-[10px] sm:text-sm font-medium text-slate-900 dark:text-white/90 truncate max-w-[110px] sm:max-w-[200px]">
                            {t.type === "buyer_to_admin" ? (
                              <span className="text-slate-900 dark:text-white font-medium">{t.buyerName}</span>
                            ) : t.type === "admin_to_seller" ? (
                              <span className="text-slate-900 dark:text-white font-medium">{t.sellerName}</span>
                            ) : (
                              <span className="text-slate-900 dark:text-white font-medium">{t.sellerName} (You)</span>
                            )}
                          </span>
