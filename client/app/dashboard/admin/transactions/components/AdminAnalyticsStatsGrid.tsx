  ...
  const cards = [
    {
      ...
      icon: <BarChart3 className="h-5 w-5 text-indigo-200" />,
    ...
    },
    {
      ...
      icon: <ArrowDownLeft className="h-5 w-5 text-emerald-200" />,
    ...
    },
    {
      ...
      icon: <ArrowUpRight className="h-5 w-5 text-amber-200" />,
    ...
    },
    {
      ...
      icon: <Wallet className="h-5 w-5 text-fuchsia-200" />,
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="h-32 rounded-2xl border border-white/10 bg-slate-900/50 animate-pulse" />
        ))}
      </div>
    );
  }
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
      {cards.map((card, idx) => (
        ...
        >
          ...
          <p className="text-white/65 text-[11px] sm:text-xs uppercase tracking-wider font-semibold mt-3">{card.title}</p>
