    ...
    <div className="min-h-screen bg-[#05050a]">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-6 animate-pulse">
        <div className="grid grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-32 bg-[#16161e] border border-white/[0.05] rounded-2xl" />
          ))}
        </div>
        <div className="h-5 w-40 bg-[#16161e] rounded-xl" />
        {[1, 2].map((i) => (
          <div key={i} className="h-24 bg-[#16161e] border border-white/[0.05] rounded-2xl" />
        ))}
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#05050a] text-white pb-20">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {statCards.map(({ label, value, icon: Icon, color, description }, i) => (
            <motion.div
              ...
              className={`bg-[#16161e] border border-white/[0.05] rounded-2xl p-5 ${glowMap[color]}`}
            >
        ...
        <motion.div
          ...
          className="bg-[#16161e] border border-white/[0.05] rounded-2xl px-6 py-4 flex flex-wrap gap-6 items-center"
        >
          <div className="flex flex-col">
            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/20 mb-0.5">Total Accounts</p>
            <p className="text-lg font-black">{accounts.length}</p>
          </div>
          <div className="w-px h-8 bg-white/[0.04] hidden sm:block" />
          <div className="flex flex-col">
            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/20 mb-0.5">Verified</p>
            <p className="text-lg font-black text-emerald-400">{verifiedCount}</p>
          </div>
          <div className="w-px h-8 bg-white/[0.04] hidden sm:block" />
          <div className="flex flex-col">
            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/20 mb-0.5">Pending Verification</p>
          ...
          {primaryAccount && (
            <>
              <div className="w-px h-8 bg-white/[0.04] hidden sm:block" />
              <div className="flex flex-col">
                <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/20 mb-0.5">Primary Bank</p>
        ...
        {accounts.length > 0 && (
          ...
          >
            <div className="px-1">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20 mb-0.5">Breakdown</p>
              <h3 className="text-sm font-black text-white/60">Registered Accounts</h3>
            </div>
            ...
            {accounts
              ...
              .map((acc, i) => (
                <motion.div
                  ...
                  className={`bg-[#16161e] border rounded-2xl px-5 py-4 flex items-center justify-between gap-4 ${
                    acc.isPrimary ? "border-cyan-500/20" : "border-white/[0.05]"
                  }`}
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                      acc.isPrimary ? "bg-cyan-500/10 text-cyan-400" : "bg-white/[0.03] text-white/20"
                    }`}>
                      <Landmark className="w-4 h-4" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        <p className="text-sm font-black text-white/80">{acc.accountHolderName}</p>
                        {acc.isPrimary && (
        ...
        {stats && stats.totalCommissionEarned > 0 && (
          <motion.div
            ...
            className="bg-[#16161e] border border-white/[0.05] rounded-2xl p-5 space-y-4"
          >
            <div className="px-1">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20 mb-0.5">Metrics</p>
              <h3 className="text-sm font-black text-white/60">Payout Efficiency</h3>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-xl px-4 py-3">
                <p className="text-[9px] font-black uppercase tracking-widest text-white/20 mb-1">Disbursed</p>
                <p className="text-base font-black text-blue-400">₹{Number(stats.totalPayoutsMade).toLocaleString()}</p>
              </div>
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-xl px-4 py-3">
                <p className="text-[9px] font-black uppercase tracking-widest text-white/20 mb-1">Retained</p>
