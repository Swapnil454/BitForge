      ...
      <div className="max-w-6xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
          ...
          >
            ...
            <div>
              ...
              {user.accountStatusUpdatedAt && (
                <p className="text-slate-400 dark:text-white/40 text-xs mt-2">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 space-y-6">
            <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-6 shadow-2xl shadow-black/50">
              <div className="flex flex-col items-center text-center">
                ...
                <div className="flex items-center justify-center gap-2 text-slate-500 dark:text-white/60 mb-4">
                  <Mail className="h-4 w-4" />
                  <span className="text-sm">{user.email}</span>
                </div>
                
                <div className="flex gap-2 mb-6">
                  <span className="px-3 py-1 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full text-xs font-medium text-slate-700 dark:text-white/80 capitalize">
                    {user.role}
                  </span>
              ...
              <div className="space-y-4 pt-6 border-t border-slate-200 dark:border-white/10 text-sm w-full">
                <div className="flex justify-between items-center p-3 bg-slate-100 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/5">
                  <span className="text-slate-400 dark:text-white/40 flex items-center gap-2"><Calendar className="h-4 w-4" /> Joined</span>
                ...
                <div className="flex justify-between items-center p-3 bg-slate-100 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/5">
                  <span className="text-slate-400 dark:text-white/40 flex items-center gap-2"><Clock className="h-4 w-4" /> Last Login</span>
                  <span className="text-slate-700 dark:text-white/80 font-medium">
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {user.role === 'seller' && (
                <div className="bg-gradient-to-br from-indigo-500/10 to-purple-500/10 border border-indigo-500/20 rounded-3xl p-6 hover:border-indigo-500/40 transition-colors shadow-lg shadow-indigo-500/5 relative overflow-hidden">
                  ...
                  <h3 className="font-medium text-slate-500 dark:text-white/60 mb-1 relative z-10">Total Products</h3>
                  <p className="text-4xl font-bold text-slate-900 dark:text-white relative z-10">{stats?.totalProducts || 0}</p>
                </div>
              )}
              ...
              <div className="bg-gradient-to-br from-cyan-500/10 to-blue-500/10 border border-cyan-500/20 rounded-3xl p-6 hover:border-cyan-500/40 transition-colors shadow-lg shadow-cyan-500/5 relative overflow-hidden">
                ...
                <h3 className="font-medium text-slate-500 dark:text-white/60 mb-1 relative z-10">Total Orders</h3>
                <p className="text-4xl font-bold text-slate-900 dark:text-white relative z-10">{stats?.totalOrders || 0}</p>
              </div>
            </div>
            ...
            <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-3xl p-6 shadow-2xl shadow-black/50">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-4 p-4 bg-slate-100 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/5 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
                  <div className={`p-3 rounded-full ${user.isVerified ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-200 dark:bg-white/10 text-slate-400 dark:text-white/40'}`}>
                    <CheckCircle2 className="h-5 w-5" />
                  </div>
                  <div>
                    <h4 className="font-medium text-slate-900 dark:text-white">Email Verification</h4>
                    <p className="text-sm text-slate-400 dark:text-white/50">{user.isVerified ? 'Email has been verified' : 'Email pending verification'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 p-4 bg-slate-100 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/5 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
                  ...
                  <div>
                    <h4 className="font-medium text-slate-900 dark:text-white">Account Status</h4>
                    <p className="text-sm text-slate-400 dark:text-white/50 capitalize">Currently {user.accountStatus}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {showBanModal && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 border border-rose-500/30 rounded-3xl p-6 max-w-md w-full shadow-2xl shadow-rose-900/20"
            >
              ...
              <div className="mb-6">
                <label className="block text-sm font-medium text-slate-500 dark:text-white/60 mb-2">
                  Reason for Ban (Required)
                </label>
                <textarea
                  value={banReason}
                  onChange={(e) => setBanReason(e.target.value)}
                  className="w-full bg-white dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-xl p-3 text-slate-900 dark:text-white focus:outline-hidden focus:border-rose-500/50 focus:ring-1 focus:ring-rose-500/50 resize-none h-24"
              ...
              <div className="flex items-center gap-3 justify-end">
                <button
                  ...
                  disabled={isProcessing}
                  className="px-4 py-2 text-slate-500 dark:text-white/60 hover:text-slate-900 dark:hover:text-white font-medium transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={isProcessing}
                  className="px-6 py-2 bg-rose-600 hover:bg-rose-500 text-slate-900 dark:text-white rounded-xl font-medium transition-all shadow-lg shadow-rose-600/20 disabled:opacity-50"
                >
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#05050a] p-6 animate-pulse">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="md:col-span-1 h-96 bg-slate-100 dark:bg-white/5 rounded-3xl" />
          <div className="md:col-span-2 space-y-6">
            <div className="grid grid-cols-2 gap-4">
