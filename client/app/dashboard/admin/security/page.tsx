            ...
            <SectionBox title="High Threat Products" accent="red" icon={<AlertTriangle className="w-4 h-4 text-red-400" />}
              ...
              {malwareStats.highThreatProducts.length === 0 ? (
                <EmptyState icon={<ShieldAlert className="w-8 h-8 text-white/10" />} text="No high-threat products detected" />
                   ...
                   <div key={p._id} className="bg-[#1c1c24] border border-red-500/20 rounded-xl p-4 sm:p-5 hover:border-red-500/30 transition-all">
                  <div className="flex justify-between items-start mb-3 gap-3 min-h-fit">
                    <div className="min-w-0 flex-1">
                      <p className="font-black text-sm sm:text-base text-white truncate">{p.title}</p>
                      <p className="text-xs text-white/50 mt-1">{p.sellerId.name}</p>
                      <p className="text-xs text-white/40">{p.sellerId.email}</p>
                    </div>
                  ...
                  <div className="flex gap-2 flex-wrap text-[10px]">
                    ...
                    {p.malwareScanDetails.detections.undetected > 0 && (
                      <span className="px-2 py-1 bg-white/[0.05] border border-white/[0.1] rounded-lg text-white/60 font-semibold">
                        {p.malwareScanDetails.detections.undetected} Undetected
                      </span>
                    )}
                  </div>
                  <div className="flex gap-2 flex-wrap mt-4 pt-3 border-t border-white/[0.05]">
                    {p.virusTotalLink && (
                      <a href={p.virusTotalLink} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-1.5 px-3 py-2 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] rounded-lg text-[11px] font-black uppercase tracking-wide transition-all">
...
function TabLoading() {
  return (
    <div className="bg-[#16161e] border border-white/[0.06] rounded-2xl p-8">
...
function MiniStat({ label, value, color }: { label: string; value: number | string; color: string }) {
  const colorMap: Record<string, { border: string; bg: string; text: string; icon: string }> = {
    cyan:    { border: "border-cyan-500/20", bg: "bg-cyan-500/[0.05]", text: "text-cyan-400", icon: "bg-cyan-500/10" },
    emerald: { border: "border-emerald-500/20", bg: "bg-emerald-500/[0.05]", text: "text-emerald-400", icon: "bg-emerald-500/10" },
    red:     { border: "border-red-500/20", bg: "bg-red-500/[0.05]", text: "text-red-400", icon: "bg-red-500/10" },
    indigo:  { border: "border-indigo-500/20", bg: "bg-indigo-500/[0.05]", text: "text-indigo-400", icon: "bg-indigo-500/10" },
    amber:   { border: "border-amber-500/20", bg: "bg-amber-500/[0.05]", text: "text-amber-400", icon: "bg-amber-500/10" },
    gray:    { border: "border-white/[0.08]", bg: "bg-white/[0.03]", text: "text-white/60", icon: "bg-white/[0.04]" },
  };
  ...
  return (
    <div className={`border rounded-2xl p-4 sm:p-5 transition-all hover:border-opacity-40 ${colors.border} ${colors.bg}`}>
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <p className="text-[11px] font-black uppercase tracking-wider text-white/40 text-right">{label}</p>
      </div>
...
function SectionBox({ title, accent, icon, count, children }: {
  ...
  return (
    <div className={`bg-[#16161e] border rounded-2xl overflow-hidden transition-all ${border[accent] || "border-white/[0.06]"}`}>
      <div className="flex items-center justify-between px-5 sm:px-6 py-4 sm:py-5 border-b border-white/[0.04] bg-white/[0.02]">
        <h2 className="text-sm sm:text-base font-black flex items-center gap-2.5">{icon}{title}</h2>
        <span className="px-3 py-1 bg-white/[0.06] border border-white/[0.08] rounded-lg text-[10px] font-black text-white/40 whitespace-nowrap ml-3">{count}</span>
      </div>
...
function EmptyState({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 sm:py-20">
      <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-white/[0.05] border border-white/[0.08] flex items-center justify-center mb-4">
        {icon}
      </div>
      <p className="text-sm sm:text-base font-black text-white/30">{text}</p>
    </div>
  );
}
