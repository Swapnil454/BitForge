    ...
    <div className="min-h-screen bg-[#05050a] text-white">
      ...
      <main className="px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl">
          {loading ? (
            <div className="flex h-64 flex-col items-center justify-center gap-4 rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm">
              <div className="h-8 w-8 animate-spin rounded-full border-2 border-cyan-500 border-t-transparent" />
              <p className="text-sm text-white/50">Fetching job details...</p>
            </div>
          ) : career ? (
            <CareerForm initialData={career} isEditing />
          ) : (
            <div className="flex h-64 flex-col items-center justify-center gap-4 rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm">
              <p className="text-white/70">Career not found</p>
              <button 
