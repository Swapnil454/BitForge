    ...
    <div className={`relative ${open ? "z-50" : "z-0"} ${className ?? ""}`}>
      ...
      {open && (
        <>
          ...
          />
          <div className="absolute z-50 mt-2 max-h-60 w-full overflow-auto rounded-xl border border-white/10 bg-[#0a0a14]/95 p-1 shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95 duration-200">
            {options.map((option) => (
              <button
                key={option.value}
                type="button"
                ...
                className={`block w-full rounded-lg px-3 py-2 text-left text-sm transition-colors hover:bg-white/10 ${
                  option.value === value ? "bg-cyan-500/10 text-cyan-300" : "text-white/70"
                }`}
              >
                {option.label}
              </button>
