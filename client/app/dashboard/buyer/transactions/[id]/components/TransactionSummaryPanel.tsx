...
export default function TransactionSummaryPanel({ transaction }: TransactionSummaryPanelProps) {
  ...
  return (
    ...
    >
      <div className="absolute -right-8 -top-10 h-40 w-40 rounded-full bg-slate-200 dark:bg-white/10 blur-2xl" />
      <div className="relative grid gap-5 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          <div className="flex flex-wrap items-center gap-2 mb-2">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{status.title}</h2>
        ...
        <div className="lg:text-right">
          <p className="text-slate-500 dark:text-white/60 text-xs uppercase tracking-wider">Amount Paid</p>
