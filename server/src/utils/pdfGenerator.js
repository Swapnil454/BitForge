...
export const generateInvoicePDF = (invoice) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      leftY += 14;
      doc.font('Helvetica').fontSize(10).text(invoice.sellerName || "BitForge Seller", leftX, leftY);
      leftY += 12;
      ...
      if (invoice.dynamicQr && invoice.dynamicQr.qrImageUrl) {
        try {
          ...
          doc.font('Helvetica').text('Invalid QR', leftX + 15, leftY + 30);
        }
      } else {
        doc.rect(leftX, leftY, 70, 70).stroke();
      ...
      leftY += 75;
      doc.font('Helvetica-Bold').text('Order Number:', leftX, leftY, { continued: true }).font('Helvetica').text(invoice.razorpayOrderId || invoice.orderId || "N/A");
      leftY += 12;
      doc.font('Helvetica-Bold').text('Order Date:', leftX, leftY, { continued: true }).font('Helvetica').text(formatDate(invoice.createdAt || invoice.invoiceDate));

      // -- RIGHT COLUMN --
      doc.font('Helvetica-Bold').fontSize(9).text('Billing Address :', rightX, rightY, { align: 'right', width: 245 });
      rightY += 12;
      doc.font('Helvetica').text(invoice.buyerName || "Valued Customer", rightX, rightY, { align: 'right', width: 245 });
      rightY += 12;
      doc.text(invoice.buyerEmail || "N/A", rightX, rightY, { align: 'right', width: 245 });
      rightY += 20;
      ...
      doc.font('Helvetica-Bold').text(invoice.productName || "Digital Product", cols[1].x + 2, rowY + 5, { width: cols[1].w - 4 });
      const titleHeight = doc.heightOfString(invoice.productName || "Digital Product", { width: cols[1].w - 4 });
